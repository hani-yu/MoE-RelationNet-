...
class BVRHead(BaseDenseHead,BBoxTestMixin):
    ...
    def init_weights(self):
        ...
        if hasattr(self, 'cls_transformer') and self.cls_transformer is not None:
            for p in self.cls_transformer.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
        if hasattr(self, 'reg_transformer') and self.reg_transformer is not None:
            for p in self.reg_transformer.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
        logger = get_root_logger()
        checkpoint_path = 'https://download.openmmlab.com/mmdetection/v2.0/retinanet/retinanet_r50_fpn_1x_coco/retinanet_r50_fpn_1x_coco_20200130-c2398f9e.pth'
        
        try:
            logger.info(f"BVRHead: 下载并加载权重: {checkpoint_path}")
            ...
            for k, v in state_dict.items():
                ...
                if k.startswith('bbox_head.'):
                    ...
                    new_k = k.replace('bbox_head.', '', 1) 
                    new_state_dict[new_k] = v
            
          
            self.bbox_head.load_state_dict(new_state_dict, strict=False)
           
         
            if hasattr(self.bbox_head, 'retina_cls'):
                bias_val = self.bbox_head.retina_cls.bias.mean().item()
    ...
    def forward(self, feats):
        ...
        kp_inputs = [feats, cls_feats, reg_feats]
        current_input = kp_inputs[self.keypoint_pos]
        
        importance_list = None # 初始化
        ...
        if self.training:
            ...
            keypoint_scores, keypoint_offsets, enhanced_feats = self.keypoint_head(current_input)
        else:
       
            outs = self.keypoint_head(current_input)
            if isinstance(outs, tuple):
           
                keypoint_scores = outs[0]
                keypoint_offsets = outs[1]
                enhanced_feats = outs[2]
                if len(outs) > 3:
                    importance_list = outs[3]
            else:
                keypoint_scores, keypoint_offsets, enhanced_feats = outs

        
        if self.keypoint_pos == 0:
      
            cls_feats, reg_feats = self.bbox_head.forward_features(enhanced_feats)
            
        elif self.keypoint_pos == 1:
          
            cls_feats = enhanced_feats
            
        elif self.keypoint_pos == 2:

            reg_feats = enhanced_feats
        ...
        if isinstance(bbox_outs, tuple):
            cls_scores = bbox_outs[0]
            bbox_preds = bbox_outs[1]
        else:
            cls_scores = bbox_outs[0]
            bbox_preds = bbox_outs[1]
    ...
    def _apply_relation(
        ...
        valid_keypoint_cfg = {
            k: v
            for k, v in self.keypoint_cfg.items()
            if k in [ "fuse_multi_level", "max_fuse_keypoint_num"]
        }
        ...
        def unpack_tuple_list_if_needed(lst, name):
            if isinstance(lst, list) and len(lst) > 0 and isinstance(lst[0], tuple):
               
                return [t for (_, t) in lst]
            return lst

        keypoint_scores = unpack_tuple_list_if_needed(keypoint_scores, "scores")
        keypoint_positions = unpack_tuple_list_if_needed(keypoint_positions, "positions")
        keypoint_inds = unpack_tuple_list_if_needed(keypoint_inds, "inds")

        try:
            keypoint_features, keypoint_positions = self.keypoint_head.get_keypoint_features(
                keypoint_feats,
                keypoint_scores,
                keypoint_positions,
                keypoint_inds,
        ...
        num_outer_heads = getattr(transformer, "num_outer_heads", 1)

        def pad_levels(lst, expected_len, fill_tensor):
            lst = list(lst)  # copy
            if len(lst) < expected_len:
                for _ in range(len(lst), expected_len):
                    lst.append(fill_tensor.clone().to(device))
            return lst

        empty_feat = torch.zeros(B, 0, C, device=device)
        empty_pos = torch.zeros(B, 0, 2, device=device)
        keypoint_features_per_level = pad_levels(keypoint_features, num_levels, empty_feat)
        keypoint_positions_per_level = pad_levels(keypoint_positions, num_levels, empty_pos)

        key_features_levels = []
        key_positions_levels = []
        for lvl in range(num_levels):
            kp_feat = keypoint_features_per_level[lvl]
            kp_pos = keypoint_positions_per_level[lvl]

           
            if not isinstance(kp_feat, torch.Tensor):
                kp_feat = torch.zeros(B, 0, C, device=device)
            if not isinstance(kp_pos, torch.Tensor):
                kp_pos = torch.zeros(B, 0, 2, device=device)

            groups_feat = [kp_feat for _ in range(num_outer_heads)]
            groups_pos = [kp_pos for _ in range(num_outer_heads)]

            key_features_levels.append(groups_feat)
            key_positions_levels.append(groups_pos)

        query_features = [feat.permute(0, 2, 3, 1).contiguous() for feat in feats]
        query_positions = []
        for i, feat in enumerate(feats):
            ...
            pos = torch.stack((x, y), dim=-1)  # H,W,2 (x,y)
            stride = (self.keypoint_head.strides[i] if hasattr(self.keypoint_head, "strides") else 1.0)
            pos = pos * stride
            pos = pos.unsqueeze(0).expand(Bf, H, W, 2)
            query_positions.append(pos)


        try:

            query_features_out, _ = transformer(
                query_features,
                query_positions,
                key_features_levels,
                key_positions_levels,
        ...
        out_feats = []
        for lvl, qf in enumerate(query_features_out):
            if isinstance(qf, torch.Tensor):
                if qf.dim() == 4:
                    # [B,H,W,C] -> [B,C,H,W]
                    out = qf.permute(0, 3, 1, 2).contiguous()
                elif qf.dim() == 3:
                    ...
                    W = feats[lvl].size(3)
                    try:
                        out = qf.reshape(N, H, W, Cq).permute(0, 3, 1, 2).contiguous()
                    except Exception:
                        out = feats[lvl]
                else:
                    out = feats[lvl]
    ...
    def get_bboxes(self,
            ...
            if hasattr(self.bbox_head, 'centerness_on_reg'):
    ...
    def aug_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        for meta in img_metas:
            ...
            if isinstance(meta, DataContainer):
                ...
                new_img_metas.append(meta.data[0])
            else:
