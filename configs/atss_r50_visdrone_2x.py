...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/VisDrone2019-DET_train_coco_start.json', #
        img_prefix=data_root + 'VisDrone2019-DET-train/images/', 
        classes=classes, 
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/VisDrone2019-DET_val_coco_start.json', #
        img_prefix=data_root + 'VisDrone2019-DET-val/images/',
        classes=classes,
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/VisDrone2019-DET_test_coco_start.json', 
...
model = dict(
    ...
    bbox_head=dict(
        ...
        keypoint_head_cfg=dict(
            ...
            loss_offset=dict(type='SmoothL1Loss', beta=1.0, loss_weight=0.2),
            loss_cls=dict(type='GaussianFocalLoss', loss_weight=0.05)),
        ...
        reg_keypoint_cfg=dict(
            ...
            with_key_score=False,
            with_relation=True),
        keypoint_cfg=dict(max_keypoint_num=100, keypoint_score_thr=0.0),
        feature_selection_cfg=dict(
            selection_method='index',
            cross_level_topk=100,
